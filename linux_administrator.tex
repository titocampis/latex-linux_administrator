...
% Table config
% \setlength{\arrayrulewidth}{1mm} % Thickness of the lines
\setlength{\tabcolsep}{18pt} % Space between columns
...
\subsection{Linux Storage Check}
\subsubsection{df}
\verb|df| stands for Disk Filesystem. It is a command used to display information about disk space usage on your file systems. It shows the total size, used space, available space, and percentage of space used for each mounted filesystem. The command is useful for checking how much free space is left on different partitions or storage devices.
Provides an overview of disk space usage for \textbf{entire filesystems or partitions}.

Basic usage, show all disk filesystems space in human-readable format:
\begin{codetemplate}{}
\begin{verbatim}
$ df-h
\end{verbatim}
...
Display output for a specific filesystem:
\begin{codetemplate}{}
\begin{verbatim}
$ df -h /dev/sda1
\end{verbatim}
...
\subsection{du}
\verb|du| stands for Disk Usage. It is a command that estimates the disk space used by files and directories. Unlike df, which operates at the filesystem level, du drills down into specific directories to show you how much space each file or subdirectory is consuming. This is useful for identifying space-hogging files and directories.
Reports on \textbf{disk space} used by specific \textbf{files or directories} within the filesystem.

Basic usage, show disk usage of the specified folder in human-readable format (if nothing specified will take the current folder)
\begin{codetemplate}{}
\begin{verbatim}
$ du -h <path/to/folder/or/file>
\end{verbatim}
\end{codetemplate}
...
Display disk usage of all files and directories in a folder:
\begin{codetemplate}{}
\begin{verbatim}
$ du -a <path/to/folder/or/file>
\end{verbatim}
\end{codetemplate}
...
\begin{codetemplate}{}
\begin{verbatim}
$ du -ch <path/to/folder/or/file> | grep total
\end{verbatim}
\end{codetemplate}
...
\begin{codetemplate}{}
\begin{verbatim}
$ du -h --max-depth=2 <path/to/folder/or/file>
\end{verbatim}
\end{codetemplate}

Exclude specific file types from the output:
\begin{codetemplate}{}
\begin{verbatim}
$ du --exclude='*.log' <path/to/folder/or/file>
\end{verbatim}
\end{codetemplate}
...
\subsection{Cron}
In Linux, \textbf{cron jobs} are automated tasks that run on a set schedule, controlled by the cron daemon \verb|crond|. 
They are incredibly useful for automating repetitive tasks like backups, updates, and monitoring scripts.

A \textbf{cron job} is a scheduled task that runs on specific times or intervals defined by the user. It uses crontab (cron table) files, where users define jobs to be executed automatically at specified times.

The \textbf{cron daemon} \verb|crond| checks these crontab files every minute to see if any jobs need to be executed.
It interprets crontab entries, which specify the exact time and frequency for each job, and then runs the command(s) accordingly.
...
Each \verb|*| can be replaced with numbers or symbols to set specific times and frequencies:
\begin{enumerate}
    \item Minute: 0-59
    \item Hour: 0-23
    \item Day of Month: 0-31
    \item Day of the week: 0-7 
\end{enumerate}

For example, \verb|0 5 * * 1 /path/to/script.sh would run /path/to/script.sh| every Monday at 5 AM.

\begin{blocktemplateI}{NOTE}
You can define the cron for intervals as well. Use \verb|*/n| when you want the job to run at a specific interval throughout the range of the field, regardless of the specific values. It’s helpful when you need recurring tasks like "every 10 minutes," "every 2 hours," or "every 3 days."
\begin{codetemplate}{}
\begin{verbatim}
...
\begin{codetemplate}{}
\begin{verbatim}
# Run the command at 0 and 30 minutes but just on the second hours
*/30 */2 * * * command
\end{verbatim}
\end{codetemplate}

You can use as well ranges and intervals within ranges: \verb|5-30/5 * * * * command|
\begin{codetemplate}{}
\begin{verbatim}
# Run the command every 5 minutes from the 5th to the 30th minute of each hour
5-30/5 * * * * command
\end{verbatim}
\end{codetemplate}

\begin{itemize}
    \item \verb|*/n|: Every \verb|n| units.	\verb|*/10| in minutes: Every 10 minutes|
    \item \verb|n|:	Exactly at n units. \verb|5| in minutes: At 5 minutes past each hour|
    \item \verb|x-y/n|:	Every n units within a range. \verb|5-30/5| in minutes: Every 5 minutes between 5 and 30 minutes past the hour
\end{itemize}
\end{blocktemplateI}

Cron jobs are typically stored in specific files depending on whether they are user-specific or system-wide. Here’s where you can find them:

\textbf{User specific crontab:} \verb|/var/spool/cron/crontabs/<username>|

So depending on the user:
\begin{itemize}
    \item \verb|/var/spool/cron/crontabs/josemanuel|: crontabs executed by user \verb|josemanuel| 
    \item \verb|/var/spool/cron/crontabs/titocampis|: crontabs executed by user \verb|titocampis|
    \item \verb|/var/spool/cron/crontabs/root|: crontabs executed by user \verb|root| 
\end{itemize}

You can check a user specific crons:
\begin{codetemplate}{}
\begin{verbatim}
josemanuel - $ less /var/spool/cron/crontabs/josemanuel
\end{verbatim}
\end{codetemplate}

\begin{codetemplate}{}
\begin{verbatim}
josemanuel - $ crontab -l
\end{verbatim}
\end{codetemplate}

You can edit a user specific crons:
\begin{codetemplate}{}
\begin{verbatim}
josemanuel - $ crontab -e
\end{verbatim}
\end{codetemplate}

\begin{blocktemplateII}{NOTE}
In user-specific cron files (e.g., \verb|crontab -e| for a specific user), there’s no user field, as the job is executed as the user who owns the crontab file.
\end{blocktemplateII}

\textbf{System wide crontab:} \verb|/etc/crontab| or \verb|/etc/cron.d/|

System wide cron are by default \textbf{executed by root}. But we can change it specifing the user to execute the cron on the \verb|/etc/crontab| file:
\begin{codetemplate}{}
\begin{verbatim}
...
\textbf{Additional cron directories:}  Some systems have directories where you can place scripts to be run at specific intervals:

\begin{itemize}
    \item \textbf{/etc/cron.hourly}
    \item \textbf{/etc/cron.daily}
    \item \textbf{/etc/cron.weekly}
    \item \textbf{/etc/cron.monthly}
\end{itemize}

\textbf{Advanced Tips}
\begin{itemize}
    \item \textbf{Logging:} Cron logs are typically found in \verb|/var/log/syslog| or \verb|/var/log/cron|, which can help in troubleshooting.
    \item \textbf{Disabling Email Notifications:}  By default, cron sends an email with output. To suppress this, redirect output to \verb|/dev/null|.
\begin{codetemplate}{}
\begin{verbatim}
...
\end{codetemplate}
    ...
    \item \textbf{Environment Variables:} You can set environment variables at the top of a crontab file, such as PATH, to ensure your scripts have the correct context.
\end{itemize} 

\subsection{lsof}
\verb|lsof| command (short for List Open Files) is a powerful utility in Unix-like operating systems that is used to list information about files that are currently opened by active processes. This includes regular files, directories, network sockets, pipes, devices, and more. Since in Unix-like systems, everything is treated as a file, lsof can show you details about active processes, network connections, open ports, and more.

Basic usage, display all open files system-wide, showing fields like the command name, process ID (PID), user, file descriptor, and the file path:
\begin{codetemplate}{}
\begin{verbatim}
$ lsof 
\end{verbatim}
...
\begin{blocktemplateIII}{WARNING}
\textbf{sudo:} Running the command as superuser (root) allows access to information about processes that might otherwise be restricted (such as those owned by other users). Without sudo, you might not be able to see details about some processes.
\end{blocktemplateIII}
...
List files opened by a specific command or process name:
\begin{codetemplate}{}
\begin{verbatim}
$ lsof -c containerd
\end{verbatim}
...
Prevent DNS lookup for network files, Without this flag, lsof would attempt to resolve IP addresses to hostnames, which can slow down the command:
\begin{codetemplate}{}
\begin{verbatim}
$ lsof -n containerd
\end{verbatim}
...
Check what processes are listening on or using specific ports and IP addresses:
\begin{codetemplate}{}
\begin{verbatim}
...
\begin{codetemplate}{}
\begin{verbatim}
$ sudo lsof -i -P -n | grep -i port
\end{verbatim}
\end{codetemplate}

\begin{itemize}
    \item \verb|-i:| ist open files related to internet connections (both network sockets and listening ports). This includes both TCP and UDP connections.
    \item \verb|-P:| Prevents lsof from converting port numbers into their associated service names. For example, without this flag, port 22 would be displayed as ssh, and port 80 would be displayed as http
    \item \verb|-n:| This flag prevents lsof from performing DNS lookups on IP addresses. Without this flag, lsof would attempt to resolve IP addresses to hostnames, which can slow down the command.
\end{itemize}
...
Kill all processes using a specific TCP port:
\begin{codetemplate}{}
\begin{verbatim}
...
\begin{itemize}
    ...
    \item \verb|-v:| verbose.
    \item \verb|-z:| compress data during the transfer to make it more efficient, if there is a local rsync it won't take effect.
...
You can specify \textbf{ssh args} like the pubkey to use, the port to connect, the proxy, etc.:
\begin{codetemplate}{}
\begin{verbatim}
$ rsync -e "ssh -p 722 -o ProxyCommand='ssh -p 22 -i ~/path/to/pub/key \ 
    -o StrictHostKeyChecking=no -W %h:%p -q username@51.15.26.3 \
    -i ~/path/to/pub/key'" \
    -avzP acampos@10.21.4.21:/home/acampos/test.txt ~/work/mysql.tar 
\end{verbatim}
\end{codetemplate}
...
Install the latest available version / upgrade a package:
\begin{codetemplate}
\begin{verbatim}
...
\subsection{Tee}
The tee command in Unix/Linux is used to read from standard input (usually the output of another command) and \textbf{write to both standard output (the terminal) and one or more files simultaneously}. It is often used when you want to save the output of a command to a file while still displaying it on the terminal for review.
...
Writing into files that require elevated privileges::
\begin{codetemplate}{}
\begin{verbatim}
cat << EOF | sudo tee file1.txt file2.txt
Este contenido
Se va a copiar tal cual
en el fichero
EOF
\end{verbatim}
\end{codetemplate}
...
The diff command is a powerful tool in Unix-like operating systems (like Linux and macOS) used to compare two files line by line. It helps identify differences between files or directories, showing what has changed, been added, or removed. It's widely used in programming, version control, and system administration.
...
\textbf{Recursively: comparing directories}
\begin{codetemplate}{}
\begin{verbatim}
...
\textbf{Compare files remotely}
\begin{codetemplate}{}
\begin{verbatim}
$ diff /local/path/file <(ssh user@remote_host: '/remote/path/file')
\end{verbatim}
\end{codetemplate}
\begin{codetemplate}{}
\begin{verbatim}
$ rsync -avnc /local/path/ user@remote_host:/remote/path/
\end{verbatim}
\end{codetemplate}

\textbf{Show Only File Differences}
\begin{codetemplate}{}
\begin{verbatim}
...
\subsection{SCP}
SCP (Secure Copy Protocol) is a command-line utility used to securely transfer files between a local host and a remote host, or between two remote hosts. It is based on the SSH (Secure Shell) protocol, ensuring that the data transferred is encrypted. SCP is commonly used in Unix-based systems like Linux and macOS, though it can also be used on Windows via tools like PuTTY.
...
\begin{codetemplate}{}
\begin{verbatim}
$ scp -rCpv ssh_config_host:/path/to/folder/ /path/to/local/folder
\end{verbatim}
\end{codetemplate}
...
\begin{blocktemplateII}{NOTE}
If we define the \textbf{Ports and privatekey} in the SSH configuration file (\verb|~/.ssh/config|),
we will be able to use the scp command without specifying Ports and privatekey each time. We don't need to specify neither user,
it will take it from the host configured on the config file.
\begin{codetemplate}{}
\begin{verbatim}
$ scp -Cpr ssh_config_host:/path/to/folder/ /path/to/local/folder
$ scp -Cpr /path/to/local/folder/ ssh_config_host:/path/to/folder
\end{verbatim}
\end{codetemplate}
\end{blocktemplateII}
...
\begin{blocktemplateI}{NOTE}
If they have different Ports (or privatekeys) you can specify it this way, but remember,
specify it on \verb|~/.ssh/config| will have the same effect:
\begin{codetemplate}{}
\begin{verbatim}
$ scp -o "Port=2222" username1@remote_host1:/path/to/file \
      -o "Port=3333" username2@remote_host2:/path/to/destination/
\end{verbatim}
\end{codetemplate}
\end{blocktemplateI}
...
To preserve file modification times, access times, and modes:
\begin{codetemplate}{}
\begin{verbatim}
$ scp -p ...
\end{verbatim}
...
To use a proxy command:
\begin{codetemplate}{}
\begin{verbatim}
$ scp -o ProxyCommand="ssh -W %h:%p username@proxy_host" ...
\end{verbatim}
\end{codetemplate}

\begin{blocktemplate}{NOTE}
If we define the \textbf{ProxyCommand} in your SSH configuration file (\verb|~/.ssh/config|),
we will be able to use the scp command without specifying ProxyCommand each time. We don't need to specify neither user,
it will take it from the host configured on the config file.
\begin{codetemplate}{}
\begin{verbatim}
$ scp -Cpr ssh_config_host:/path/to/folder/ /path/to/local/folder
...
To know listening ports on localhost:
...
\begin{blocktemplate}{NOTE}
Nmap by defaul scan only \textbf{TCP ports}, but you can specify the option \verb|-sU| and use it with all the rest of flags.
\end{blocktemplate}
