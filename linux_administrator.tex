...
\textbf{Display only a total for each argument (not recursively):}
\begin{codetemplate}
\begin{verbatim}
$ du -sh
\end{verbatim}
...
\subsection{wc}
\verb|wc| (short for \textbf{word count}) is a command-line utility in Unix/Linux systems used to count lines, words, and characters (or bytes) in files or input streams. It's often used for quick text statistics and file size information.
...
To see \verb|<lines> <words> <characters>| of file.txt:
\begin{codetemplate}{}
\begin{verbatim}
$ wc file.txt
\end{verbatim}
...
\textbf{Using wc with Pipelines:} You can pipe the output of other commands into wc. For example, to count the number of lines in a command's output:
\begin{codetemplate}{}
\begin{verbatim}
$ ls -l | wc -l
\end{verbatim}
...
The simplest usage of wget is downloading a single file from a URL:
\begin{codetemplate}{}
\begin{verbatim}
...
For downloading a .tar.gz file and extracting it in one step:
\begin{codetemplate}{}
\begin{verbatim}
...
\end{codetemplate}
\begin{itemize}
    \item \verb|-q|: Quiet mode (optional, suppresses wget output).
    \item \verb|-O-| (not zero): Outputs the downloaded content directly to stdout (the hyphen - represents stdout)
\end{itemize}

Resuming an Interrupted Download If a download is interrupted, you can resume it using the -c flag:
\begin{codetemplate}{}
\begin{verbatim}
...
Download Files in the Background To let the download happen in the background, freeing up the terminal for other commands:
\begin{codetemplate}{}
\begin{verbatim}
$ wget -b https://ftp.postgresql.org/pub/source/v9.6.6/postgresql-9.6.6.tar.gz
\end{verbatim}
\end{codetemplate}

Download Multiple Files You can download multiple files at once by specifying URLs in a text file and using the -i flag:
\begin{codetemplate}{}
\begin{verbatim}
...
Log Output to a File: You can log the output to a file for later inspection using the -o option:
\begin{codetemplate}{}
\begin{verbatim}
...
To suppress all output except for errors, use the --no-verbose option:
\begin{codetemplate}{}
\begin{verbatim}
$ wget -q --no-verbose https://example.com/file.zip
\end{verbatim}
\end{codetemplate}
...
\begin{blocktemplateII}{NOTE}
All modifications you do with \verb|set| will only apply to the current vim opened file, if you repen again vim, it won't keep it. In order to keep configurations
modify:
\begin{codetemplate}
\begin{verbatim}
~/.vimrc
...
\end{verbatim}
\end{codetemplate}
To replace all the occurrences of a word in the full file (with confirmation before each replacement)
\begin{codetemplate}
\begin{verbatim}
:%s/search/replace/gc
\end{verbatim}
...
\subsection{TLS Communication (Certificates)}
...
As well, you can use \textbf{certbot (let's encrypt)} (like a CA), but more quicklier and easier to do, take a look on \href{https://certbot.eff.org/}{CetBot Official Documentation}.
...
\begin{codetemplate}{}
\begin{verbatim}
$ nmap -O <hostname_or_ip> # it is uppercase "o" not zero
\end{verbatim}
\end{codetemplate}
...
Specify the Maximum Number of Hops (routers), by default set to 30:
\begin{codetemplate}
\begin{verbatim}
$ traceroute -m <max_hops> <hostname_or_ip>
\end{verbatim}
\end{codetemplate}


\subsection{My Traceroute (MTR)}
\verb|mtr| (My Traceroute) is a powerful network diagnostic tool that combines the functionality of two essential network utilities: \verb|ping| and \verb|traceroute|. It provides real-time network performance statistics, helping users identify potential network issues like packet loss, latency, and routing problems between the local machine and a remote host.

Unlike traditional \verb|ping| or \verb|traceroute| which give static outputs, mtr runs continuously, providing updated information and live network analysis. It is especially useful for troubleshooting network connectivity issues or analyzing the performance of network routes over time.
...
\begin{codetemplate}
\begin{verbatim}
$ mtr <hostname_or_ip> -c <number_of_pings>
\end{verbatim}
\end{codetemplate}

Specify the Maximum Number of Hops (routers):
\begin{codetemplate}
\begin{verbatim}
$ mtr -m <max_hops> <hostname_or_ip>
\end{verbatim}
\end{codetemplate}

\begin{blocktemplateII}{NOTE}
Running mtr as \textbf{root} enables the program to \textbf{send ICMP packets} for more accurate results. Without root privileges, mtr sends UDP packets, which may behave differently.
\begin{codetemplate}
\begin{verbatim}
...
Specify intervals (in seconds) between packets:
\begin{codetemplate}
\begin{verbatim}
$ mtr -i <seconds> <hostname_or_ip>
\end{verbatim}
\end{codetemplate}
...
\begin{codetemplate}{}
\begin{verbatim}
$ whois --verbose <hostname_or_ip>
\end{verbatim}
\end{codetemplate}

Specify a particular WHOIS server to query, which can be useful if the default server doesn’t provide the desired information:
\begin{codetemplate}{}
\begin{verbatim}
...
If you only want a short summary of the information:
\begin{codetemplate}{}
\begin{verbatim}
