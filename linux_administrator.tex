...
\begin{enumerate}
    \item If we execute it with \verb|./|
    \begin{itemize}
        \item First, we need to give it permissions.
    \begin{figure}[H]
        \centering
        \includegraphics[scale=0.45]{pictures/image13.PNG}
        \label{./bash}
    \end{figure}
        \item Secondly, as we anticipated, the changes are not saved in the terminal, since a subprocess is executed in the background which never merges system-level changes into the parent process.    \begin{figure}[H]
        \centering
        \includegraphics[scale=0.6]{pictures/image12.PNG}
        \label{./bash 2}
    \end{figure}
    \end{itemize}
    \item If we execute it with \verb|bash/sh|
        \begin{itemize}
            \item In this case, permissions are not necessary, as it is passed as an argument to the bash executable.
            \item Secondly, as we anticipated, the same thing happens as with `./`, the changes are not saved in the terminal (parent process).
    \begin{figure}[H]
        \centering
        \includegraphics[scale=0.45]{pictures/image14.PNG}
        \label{bash scrip.sh}
    \end{figure}
    \end{itemize}
    \item \textbf{If we execute it with source}
        \begin{itemize}
            \item In this case, permissions are not necessary, as it is passed as an argument to the source executable.
            \item Secondly, as we anticipated, the changes are transmitted to the parent process! And therefore they are saved in our terminal.
    \begin{figure}[H]
        \centering
        \includegraphics[scale=0.45]{pictures/image15.PNG}
...
\subsubsection{Hacer y deshacer}
...
\subsubsection{Borrar lineas enteras}
\begin{codetemplate}
...
\subsubsection{Buscar Ocurrencias}
\textbf{Para buscar}
\begin{codetemplate}
\begin{verbatim}{vim editor - Normal Mode}
/<palabra>
...
\paragraph{Use the Asimetric Key authentication method}
